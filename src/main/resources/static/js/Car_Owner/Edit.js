...
// Upload bằng click
    imageUploadDefault.each(function (index) {
        ...
        jQuery(this).on('change', function () {
            if (index == 0) {
                data.append("file3", this.files[0]);
            } else if (index == 1) {
                data.append("file4", this.files[0]);
            } else if (index == 2) {
                data.append("file5", this.files[0]);
            } else if (index == 3) {
                data.append("file6", this.files[0]);
            }
...
//Xóa file đã tải lên
    removeImageButton.each(function (index) {
        jQuery(this).click(function () {
            if (index == 0) {
                data.append("file4", undefined);
            } else if (index == 1) {
                data.append("file5", undefined);
            } else if (index == 2) {
                data.append("file6", undefined);
            } else if (index == 3) {
                data.append("file7", undefined);
            }
    ...
    saveBtn.on("click", function (){
        ...
        if ((editDetail == true) && (editPricing == true)) {
            data.append("carId", carId.val());
            data.append("mileage", mileage.val());
            data.append("fuelConsumption", fuelConsumption.val());
            data.append("address", address.val());
            data.append("description", description.val());
            data.append("additionalFunction", additionalFunction.val());
            data.append("terms", terms.val());
            data.append("basePrice", basePrice.val());
            data.append("deposit", deposit.val());
            data.append("carStatus", carStatus.val());
            jQuery.ajax({
                url: "http://localhost:8080/mycar/editdetails",
                type: "POST",
                data: data,
                processData: false,
                contentType: false,
                success: function (response) {
                    if (response == "unauthorized") {
                        ...
                        window.location.href = "/mycar/edit?carid=" + carId.val();
                    }
                },
